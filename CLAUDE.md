...
- Carried C is one translation unit for the runtime and one per grammar, all
  under `grammars/`, and cannot be fewer: the runtime and a grammar each
...
- `spin test --regen` writes that snapshot by running the same file under
  ...
  no grammar, so what is written by hand is whichever tests read source or ask
  for a binding — a list that moves with every reading added, and one to work
  out from the requires rather than to remember. Prose is the one reading a build carries that needs no
...
- `--regen` takes the same file list, so name the test to rewrite — and name
  one that can be. Given none it rewrites every snapshot, and given a
