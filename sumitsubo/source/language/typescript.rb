...
module Sumitsubo
  module Source
    class Language
      # TypeScript, read through the grammar this build links in.
      #
      # A name is the declaration reference TypeDoc resolves: `Charge#settle`
      # for a member reached through a value, `Charge.open` for one reached
      # through the class, and the dotted path for whatever a namespace or the
      # module holds. That is JavaScript's namepath with what TypeScript adds
      # written under the same marks — an interface's method is reached through
      # a value like any other member.
      #
      ...
      class Typescript
        ...
        def reads?(path)
          [".ts", ".mts", ".cts"].include?(path.extname)
        end
